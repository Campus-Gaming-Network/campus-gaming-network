  ...
  return (
    <Box safeArea flex={1} p="2" py="8" w="90%" mx="auto">
      <VStack alignItems="center" mt={100} space={6}>
        <Image source={logo} style={{ height: 100, width: 100 }} alt="Campus Gaming Network logo" />
        <Heading size="lg" fontWeight="bold">
          Campus Gaming Network
        </Heading>
        <Text>({user ? user.email : null})</Text>
      </VStack>
      <Button mt={6} colorScheme="orange" onPress={() => navigation.navigate('LogIn')} _text={{ fontWeight: 'bold' }}>
        Log In
      </Button>
    </Box>
  );
