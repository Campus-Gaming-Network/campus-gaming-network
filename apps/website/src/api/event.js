...
export const getEventDetails = async (id) => {
  ...
  try {
    const eventDoc = await firebaseAdmin.firestore().collection('events').doc(id).get();
...
export const getEventUsers = async (id, limit = DEFAULT_USERS_LIST_PAGE_SIZE, page = 0) => {
  let users = [];
...
export const incrementEventPageViews = async (id) => {
  const eventRef = firebaseAdmin.firestore().collection(COLLECTIONS.EVENTS).doc(id);

  try {
    await eventRef.set({ pageViews: firebaseAdmin.firestore.FieldValue.increment(1) }, { merge: true });
