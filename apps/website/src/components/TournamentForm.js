...
// Dynamic Components
const DeleteTournamentDialog = dynamic(() => import('src/components/dialogs/DeleteTournamentDialog'), { ssr: false });
...
const TournamentForm = (props) => {
  ...
  const [isDeletingTournamentAlertOpen, setDeletingTournamentAlertIsOpen] = useBoolean();
  const handleFieldChange = React.useCallback((e) => {
    formDispatch({ field: e.target.name, value: e.target.value });
  }, []);
  const hasErrors = React.useMemo(() => !isEmpty(props.errors), [props.errors]);
  const pageTitle = React.useMemo(
    () => (props.state === 'edit' ? `Edit ${props.tournament.name}` : 'Create Tournament'),
        ...
        <Stack as="form" spacing={2} onSubmit={handleSubmit}>
          <Flex alignItems="center" flexWrap="wrap">
            <Box pb={12}>
              ...
              <Flex align="center">
                <Img src="/challonge-logo.png" alt="Challonge" title="Challonge" h={4} mr={2} />
                <Text fontSize="sm" textTransform="" fontWeight="bold" color="gray.500">
                  Powered by <OutsideLink href="https://challonge.com/">Challonge! &trade;</OutsideLink>
                </Text>
              </Flex>
            </Box>
            <Spacer />
            {props.state === 'edit' ? (
              <Button variant="ghost" colorScheme="red" size="lg" onClick={setDeletingTournamentAlertIsOpen.on}>
                Delete tournament
              </Button>
            ) : null}
          </Flex>
            ...
            <Stack spacing={6} p={8}>
              ...
              <FormControl isInvalid={props.errors.description}>
                ...
                <FormHelperText id="description-helper-text">
                  Describe your tournament in fewer than {MAX_DESCRIPTION_LENGTH.toLocaleString()} characters.{' '}
          ...
          <Card as="fieldset" p={0} mb={32}>
            ...
            <Stack spacing={6} p={8}>
              <FormControl isRequired isInvalid={props.errors.tournamentFormat}>
                <FormLabel htmlFor="tournamentFormat" fontSize="lg" fontWeight="bold">
                  Tournament Format
                </FormLabel>
                <Select
                  id="tournamentFormat"
                ...
                <FormErrorMessage>{props.errors.tournamentFormat}</FormErrorMessage>
              </FormControl>
              {formState.tournamentFormat === 'single elimination' ? <SingleEliminationFormat /> : null}
              {formState.tournamentFormat === 'double elimination' ? <DoubleEliminationFormat /> : null}
              {formState.tournamentFormat === 'round robin' ? <RoundRobinFormat /> : null}
          ...
          <Box pt={16}>
            <Button colorScheme="brand" type="submit" size="lg" w="full" disabled={props.isSubmitting}>
              {props.isSubmitting ? 'Submitting...' : props.state === 'edit' ? 'Edit Tournament' : 'Create Tournament'}
            </Button>
          </Box>
        </Stack>
      </Article>
...
const DoubleEliminationFormat = () => {
  return (
    <React.Fragment>
      <FormControl>
        <FormLabel htmlFor="grandFinalsModifier" fontSize="lg" fontWeight="bold">
          Grand Finals
        </FormLabel>
        <RadioGroup
    ...
    <React.Fragment>
      ...
      {POINT_INPUT_OPTIONS.map((option) => (
        <FormControl d="flex" alignItems="center" justifyContent="space-between">
          ...
          <NumberInput defaultValue={option.defaultValue} precision={2} step={0.25}>
            <NumberInputField />
            <NumberInputStepper>
              <NumberIncrementStepper />
              <NumberDecrementStepper />
